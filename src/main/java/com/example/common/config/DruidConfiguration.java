...
/**
 * Druid监控配置, 如果同时进行了编程式的注入和配置的注入，配置的就无效了,
 * 这里选择编程式注入,是为了使用druid的SQL监控(filters), 1.4.1的版本这个功能不支持配置注入
...
//        servletRegistrationBean.addInitParameter("deny","192.168.2.102");
        //登录查看信息的账号密码.
